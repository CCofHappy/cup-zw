...
export default {
	...
	methods: {
		initData() {
			...
			let params = {
				apiUrl: this.config.mallApi + "order/logistics/" + this.$route.query.id,
				apiMethod: "get",
			}
			if (this.$router.query.from == 'supply') {
				params.apiUrl= this.config.mallApi + "supplier/logistics/" + this.$route.query.id;
			}
			that.ajaxData(params, (res) => {
